...
func TestRPCMain(t *testing.T) {
	...
	for i := 0; i < 5; i++ {
		...
		cmds[i] = exec.Command("./server", port, strconv.Itoa(i))
		err := cmds[i].Start()
		if err != nil {
...
func TestRPC_BasicSequential(t *testing.T) {
	...
	data := "Cloud fun"
	m, err := cl.write("cs741net", data, 0)
	if m.Kind == 'L' {
		Leaderport = getLeaderport(LeaderId)
		cl = mkClient(t)
...
func TestRPC_BasicSequential2(t *testing.T) {
	...
	closednode := LeaderId
	fmt.Println(closednode)
	cmds[LeaderId].Process.Kill()
	...
	cmds[closednode] = exec.Command("./server", strconv.Itoa(8000+closednode), strconv.Itoa(closednode))
	err = cmds[closednode].Start()
	if err != nil {
		fmt.Println("starting", err)
	}
	...
	m, err = cl.read("cs733net")
	expect(t, m, &Msg{Kind: 'C', Contents: []byte(data2)}, "read my cas", err)
	fmt.Println("testcase6")

	// Expect Cas to fail with old version
	m, err = cl.cas("cs733net", version1, data, 0)
	expect(t, m, &Msg{Kind: 'V'}, "cas version mismatch", err)
	fmt.Println("testcase7")

	// Expect a failed cas to not have succeeded. Read should return data2.
	m, err = cl.read("cs733net")
	expect(t, m, &Msg{Kind: 'C', Contents: []byte(data2)}, "failed cas to not have succeeded", err)
	fmt.Println("testcase8")

	// delete
	m, err = cl.delete("cs733net")
	expect(t, m, &Msg{Kind: 'O'}, "delete success", err)
	fmt.Println("testcase9")
	// Expect to not find the file
	m, err = cl.read("cs733net")
	expect(t, m, &Msg{Kind: 'F'}, "file not found", err)
	fmt.Println("------->>")
...
func mkClient(t *testing.T) *Client {
	var client *Client
	raddr, err := net.ResolveTCPAddr("tcp", "localhost:"+strconv.Itoa(Leaderport))
	if err == nil {
...
func parseFirst(line string) (msg *Msg, err error) {
	...
	case "ERROR_REDIRECTING":
		msg.Kind = 'L'
		LeaderId, _ = strconv.Atoi(fields[1])

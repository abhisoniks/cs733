...
//var logDir *log.Log
func (rn *RaftNode) Append(data []byte) { //This functioin handles every Append request coming to this node
	rn.sm.SMchanels.clientCh <- APPEND{data} //It passes this Append request to stateMAchine through clientCh channel
...
func initialize(serv cluster.Server, id int) *RaftNode {
	...
	switch id {
	case 0:
		raft_config = &Config{0, "log0", 800, 100}
	...
	case 1:
		...
		logDir1, err := log.Open("log1")
		checkError(err)
		raftN = &RaftNode{config: raft_config, timerFlag: 1, sm: temp_sm, logDir: logDir1}
	case 2:
		...
		checkError(err)
		raftN = &RaftNode{config: raft_config, timerFlag: 1, sm: temp_sm, logDir: logDir2}
	case 3:
	...
	go raftN.check_Inbox(serv)
	go raftN.check_SMAction(serv)
	go temp_sm.processEvent()

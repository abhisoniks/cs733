...
func (sm *STATEMACHINE) Append(appendEVENT APPEND) {
	switch sm.STATE {
	case "Follower":
		c := COMMIT{DATA: appendEVENT.A, ERROR: 404}
...
func (sm *STATEMACHINE) APPENDENTRIESREQ(AER_EVENT APPENDENTRIESREQ) {
	...
	case "Follower":
		...
		if AER_EVENT.TERM >= sm.CURRENTTERM && AER_EVENT.ENTRIES == nil { 
			if prevINDEX == AER_EVENT.PREVLOGINDEX { 
				t := sm.getElectionTime()
				sm.SMchanels.Action <- ALARM{TIME: t}
			...
			for k := match + 1; k < len(AER_EVENT.ENTRIES); k++ { 
